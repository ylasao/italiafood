...
const OpenImg = ({
  ...
  return (
    <div className="fixed z-[100]  cursor-pointer top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center">
      <CiCircleRemove
        onClick={handleOpen}
        className="text-[4rem]  transition-200 duration-300 ease-in-out text-white  absolute cursor-pointer right-[3rem]  md:right-20 top-[30%] md:top-[10%]  "
      />
      <GrFormPrevious
        onClick={handleClickPrevious}
        className="text-[3rem] hover:bg-black transition-200 duration-300 ease-in-out text-white bg-black/50 absolute cursor-pointer left-10  top-[50%]  "
      />
